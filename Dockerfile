...
# RUN cd /tmp && tar -xzf protobuf.tar.gz && \
#   cd protobuf-"$PROTOBUF_VERSION" && ./autogen.sh -s && ./configure && make && make install && \
#   cd / && rm -rf /tmp/* && rm -rf /var/cache/apk/* /tmp/*

RUN \
  apk add --update --no-cache gcc g++ musl-dev make pkgconfig libtool autoconf automake libuuid file curl && \
  cd /tmp && tar -xzf protobuf.tar.gz && \
  cd protobuf-"$PROTOBUF_VERSION" && ./autogen.sh -s && ./configure && make && make install && \
  apk del gcc g++ musl-dev make pkgconfig libtool autoconf automake libuuid file curl && \
  cd / && rm -rf /tmp/* && rm -rf /var/cache/apk/* /tmp/*

#RUN mkdir -p /usr/src/protoc-gen-flow
#COPY . /usr/src/protoc-gen-flow
#
#WORKDIR /usr/src/protoc-gen-flow
#
#RUN npm install
